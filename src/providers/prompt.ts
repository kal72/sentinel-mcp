...
export function buildSecurityAnalysisPrompt(
  ...
  return [
    ...
    JSON.stringify(pruneTestResults(testResults), null, 2),
    '',
    'For each test case, determine if the attack was EXPLOITED (VULNERABLE) by looking for:',
    '  * Data Leakage: Inclusion of sensitive data (other users\' records, internal IDs, database fragments).',
    '  * Injection Success: Result of code execution (e.g. output of "whoami" or "ls") or structural changes.',
    '  * XSS Reflection: Is the attack script (e.g. <script>) reflected back in the body without proper escaping?',
    '  * IDOR Success: Accessing or modifying a resource that belongs to a different user ID than the requester.',
    '  * SSRF/File Access: Response containing internal metadata (169.254...) or local files (/etc/passwd).',
    '  * Error Leakage: Detailed SQL errors, stack traces, ORM messages, or server version info.',
    '  * Auth Bypass: Request succeeding (2xx) despite invalid, expired, or missing tokens/headers.',
    '',
    'CRITICAL: A 200 OK status code is a FAILURE if the body contains evidence of successful exploitation (Reflection, Data Leak, etc.).',
    '',
